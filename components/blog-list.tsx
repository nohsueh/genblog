...
async function BlogListContent({ lang, dictionary, group }: BlogListProps) {
  // Only fetch blogs that have a group specified
  const posts = await getPublishedBlogs(1, 10, group)
        ...
        <Card key={post.analysisId} className="overflow-hidden">
          ...
          <CardContent className="p-4">
            <CardTitle className="line-clamp-2 mb-2">
              <Link href={`/${lang}/blog/${post.analysisId}`} className="hover:underline">
                {post.analysis.title}
              </Link>
            </CardTitle>
            <div className="flex items-center gap-2 mb-2">
              {post.metadata?.group && <Badge variant="outline">{post.metadata.group}</Badge>}
            </div>
